...
export class DevolucaoService {
  ...
  async insertDevolucao(devolucao: NovaDevolucao): Promise<Devolucao> {
    ...
    cobrança.valor = 30;
    const ciclista = await this.ciclistaService.getCiclistaByID(
      devolucao.ciclista,
    );
    ...
    const Aluguel = await this.aluguelService.getAluguelByCiclista(
      devolucao.ciclista,
    );
    ...
    const fimCobrança = await this.realizaCobrança(
      devolucao,
      alugado.horaFim,
      Aluguel.horaInicio,
    );
    ...
    const updateAluguel = await this.aluguelService.updateAluguel(
      devolucao.ciclista,
      Aluguel,
    );
    console.log('tranca', alugado);
    const devolucaoResult = await this.devolucaoRepository.insertDevolucao(
      alugado,
    );
    return devolucaoResult;
  }

  async realizaCobrança(
    ...
    const diffInMinutes =
      Math.abs(currentDate2.getTime() - currentDate1.getTime()) / 60000; // Calcula a diferença em minutos
